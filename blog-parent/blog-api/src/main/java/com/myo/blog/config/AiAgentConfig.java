...
public class AiAgentConfig {
    ...
    public ReactAgent blogAdminAgent(UserAiTools userAiTools, ArticleAiTools articleAiTools, DatabaseAiTools databaseAiTools, RedisTemplate<String, String> redisTemplate) {
        ...
        // 4. 构建并返回 ReactAgent
        ReactAgent blogAdminAgent = ReactAgent.builder()
                ...
                .systemPrompt(
                        ...
                        "你是这个博客系统的后台管理智能助手，只服务于有权限的管理员。\n" +
                                ...
                                // ========== 具调用强制原则 ==========
                                "【工具调用强制原则】：\n" +
                                "凡是涉及用户操作（封禁、解封、警告、查询用户）、文章操作、评论操作等任何管理类指令，必须调用对应工具执行，严禁凭自身知识或上下文直接回答「能不能做」、「有没有权限」这类问题。\n" +
                                "判断当前用户有没有权限做某件事，唯一正确方式是调用工具让系统自己返回结果，不得自行推断。\n" +
                                ...
                                "6. 当某项操作因权限不足被拒绝时，严禁向用户透露还有其他方式或备用方案可以绕过。只能回复『您当前身份没有该操作权限哦～』，不得做任何延伸说明或引导。\n" +
                                "7. 严禁在回复中主动说出当前登录用户的账号名称，账号属于系统内部标识，不得对外展示。\n" +
                                "8. 严禁向用户解释系统内部的拦截机制、校验逻辑或安全规则细节，遇到敏感操作只需告知结果，不解释原因。\n" +
                                "9. 严禁在回复中主动说明系统内部的过滤规则，例如『已自动过滤您本人账号』、『已隐藏高权限用户』等，这些是系统内部逻辑，不得对外展示，直接展示结果即可。"
                )
